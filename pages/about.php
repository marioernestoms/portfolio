<?php
/**
 * Template Name: About
 *
 * The template for displaying pages with sidebar.
 *
* @package marioernestoms
 */

?>

<section id="section-about" class="about-section">
    <div class="container">
        <div class="row">
            <h1>About Me</h1>
            <h2 class="subtitles highlight">This is the place to know a little bit about me and my work.</h2>
            <div class="about-us col-md-12" style="padding-top: 3%;">
                <div class="col-sm-7 wow fadeInLeft" data-wow-duration="1000ms" data-wow-delay="300ms" style="border-right: 1px dotted #ccc">
                    <div class="row">
                        <div class="col-sm-5">
                            <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/eu.jpg" class="img-responsive" alt="">
                        </div>
                        <div class="col-sm-7">
                            <h3 class="top-zero">Welcome, I'm Mario Ernesto!</h3>
                            <p>Full Stack Developer from Rio de Janeiro, Brazil. I work with all kinds of projects from simple to complex, always seeking the best experience of interaction with the users.</p>
                            <p>My objective is working with clean coding that's use the best code standards practices, I have full domain of PHP, HTML5, CSS3, JavaScript, Performance, SEO.</p>
                            <p>My specialty is full stack development and WordPress is my passion. In the last 4 years I decided work only with WordPress projects, I build themes and Plugins with a perfect balance between code standard and best performance.</p>
                            <p>During my spare time together with my friends of the WP-Rio, I organized the monthly WordPress Meetups in Rio de Janeiro and during the last 3 years, I participated as a volunteer and organizer of WordCamps, an official conference authorized by the foundation that takes place in dozens of cities around the world. Helping to organize the events besides being a great learning is something that I am very proud to do and I do with great affection.</p>
                            <div class="ui stacked spaced segment">
                                <p class="available-frelancer btn btn-success"><i class="fa fa-thumbs-up" aria-hidden="true"></i> Available for Freelance</p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-sm-5 wow fadeInRight" data-wow-duration="1000ms" data-wow-delay="300ms">
                    <h3 class="top-zero"><i class="fa fa-graduation-cap" aria-hidden="true"></i> My skills</h3>
                    <div class="our-skills">
                        <p>► 8+ YEARS BUILDING WEBSITES AND WEB SYSTEMS</p>
                        <ul>
                            <li class="single-skill">
                                WordPress Theme and Plugins Development
                                <div class="progress">
                                    <div class="progress-bar progress-bar-primary six-sec-ease-in-out" role="progressbar"  data-transition="50">4+ Years</div>
                                </div>
                            </li>
                            <li class="single-skill">
                                PHP Back-end Development
                                <div class="progress">
                                    <div class="progress-bar progress-bar-primary six-sec-ease-in-out" role="progressbar"  data-transition="40">3+ Years</div>
                                </div>
                            </li>
                            <li class="single-skill">
                                Planning Front-end Infrastructures
                                <div class="progress">
                                    <div class="progress-bar progress-bar-primary six-sec-ease-in-out" role="progressbar"  data-transition="100">8+ Years</div>
                                </div>
                            </li>
                        </ul>

                        <p>► SPECIALTIES</p>
                        <ul>
                            <li class="single-skill">Full domain with HTML/CSS/JS</li>
                            <li class="single-skill">PHP development with MySQL</li>
                            <li class="single-skill">Preprocessor (Sass)</li>
                            <li class="single-skill">Task Runners (Gulp/Grunt)</li>
                            <li class="single-skill">Package Managers (Bower)</li>
                            <li class="single-skill">Source Control (Git, Bitbucket)</li>
                            <li class="single-skill">Responsive and Cross-browser sites and systems</li>
                            <li class="single-skill">Web Servers with Nginx, PHP-FPM, MariaDB ( AWS, Linode, Digital Ocean)</li>
                            <li class="single-skill">Track Work (Trello, Asana)</li>
                            <li class="single-skill">Time Track (Hubstaff, WakaTime)</li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
